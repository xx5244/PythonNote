...
+ ## 正則表達式
    ...
    + ### 範例
        ...
        另外，非貪婪模式基本上不會單獨使用(不然基本上都會是空的)，會搭配其它匹規則來使用(例:前/後加其它的規則)，而會先去匹配貪婪模式的規則後才會匹配非貪婪模式的規則
        """
        ...
        def test(strings, pat):
            ...
            for string in strings:
                try:
                    result.append((re.findall(f'(?:[^A-z])({string})(?:[^A-z])', pat, re.S)[0]))    # 取第0個是避免重複值，且取list的值好放入最後的list中
                except:
                    continue
            return result
